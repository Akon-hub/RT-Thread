...
static rt_err_t drv_pwm_set(TIM_HandleTypeDef *htim, struct rt_pwm_configuration *configuration)
{
    ...
    /* Convert nanosecond to frequency and duty cycle. 1s = 1 * 1000 * 1000 * 1000 ns */
    tim_clock /= 1000000UL;//90
    period = (unsigned long long)configuration->period * tim_clock / 1000ULL ;//9k ns
    psc = period / MAX_PERIOD + 1;
    period = period / psc;//9 000
    ...
    pulse = (unsigned long long)configuration->pulse * tim_clock / psc / 1000ULL;//4500
    if (pulse < MIN_PULSE)
    {
        pulse = MIN_PULSE;
    }
